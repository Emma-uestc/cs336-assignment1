"""
Implement a BPE trainer with multiprocessing optimization.
"""
...
def _pretokenize_chunk(args: Tuple) -> Dict[str, int]:
    """
    ...
    # each child process independently reads its own chunk text
    with open(file_path, 'r', encoding='utf-8') as f:
        f.seek(start)
        text = f.read(end-start)

    # compile the pattern in each child process
    # because child process cannot share the compiled pattern with the parent process
